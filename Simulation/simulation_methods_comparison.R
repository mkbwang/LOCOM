######
# all possible choices of set up combinations
######
library(tidyr)
scenarios <- c("binary_Y_only")
foldchanges <- seq(2, 4, 0.5)
seeds <- seq(1, 100)
allsettings <- crossing(scenarios, foldchanges, seeds)

# specific setting
ID <- as.integer(Sys.getenv('SLURM_ARRAY_TASK_ID'))
ID <- 494
scenario <- allsettings$scenarios[ID]      # binary_Y_only, continuous_Y_only, binary_Y_binary_C, binary_Y_continuous_C, continuous_Y_binary_C, continuous_Y_continuous_C
causal.type <- 1   
n.sam <- 100        
beta <- allsettings$foldchanges[ID]         
have.bias <- FALSE
n.cores <- 2
...
code_folder <- '/home/wangmk/MDAWG/LOCOM/Simulation'
...
library(POLDA)
library(LOCOM)
source(file.path(code_folder, "ancom_v2.1.R"))
...
pi <- read.table(file.path(code_folder, "input_throat/fit_dirmult_pi.txt"), 
                 header=FALSE, as.is=TRUE)[,1]
n.otus <- length(pi)
ref.otu <- which.max(pi) # 468


i.seed <- allsettings$seeds[ID]
set.seed(i.seed)
...
if (scenario == "binary_Y_only" | scenario == "continuous_Y_only"){  # no confounder
    ...
    if (causal.type == 1){
        available_taxa <- which(pi >= 0.005)
        otu.Y.only <- sample(available_taxa, 20)
    }
    if (causal.type == 2){
...
summarize_otu_results <- function(qvalue, causal.otus, noncausal.otus, fdr.target=0.2) {
    ...
    if (n.otu > 0) {
        ...
        spe = 1 - sum(otu.detected %in% noncausal.otus)/length(noncausal.otus)
    ...
    out = list(n.otu=n.otu, sen=sen, spe=spe, fdr=fdr)
...
res.locom <- locom(otu.table = otu.table.sim.filter, Y = Y, C = C, 
                   ...
                   n.perm.max = 20000, n.rej.stop = 100, n.cores = n.cores)	
...
if (scenario == "binary_Y_only"){
    ...
    aldex2.qvalue <- p.adjust(res.aldex2$`Y:pval`, method="BH")
}
...
if (scenario == "binary_Y_only" | scenario == "binary_Y_binary_C" | scenario == "binary_Y_continuous_C"){
    ...
    detected.otu.ancom <- which(res.ancom$out[, paste("detected_", 0.7, sep = "")]== TRUE)
...
if (scenario == "binary_Y_only" | scenario == "binary_Y_binary_C" | scenario == "binary_Y_continuous_C" | scenario == "continuous_Y_only"){
  ###########
  # ANCOM-BC
  ###########
  ...
  if (scenario == "continuous_Y_only") group_variable <- NULL
  ...
  res.ancombc <- ancombc(phyloseq = phylo.obj, formula = formula,
                         ...
                         group = group_variable, struc_zero = FALSE, neg_lb = FALSE,
                         tol = 1e-5, max_iter = 100, conserve = TRUE, alpha = 0.1, global = FALSE)
  
  otu.ancombc <- summarize_otu_results(qvalue = res.ancombc$res$q_val[,3], causal.otus = causal.otus.filter, noncausal.otus = noncausal.otus.filter,
                                       fdr.target = 0.1)
}
...
if (scenario == "binary_Y_only" | scenario == "continuous_Y_only"){
  ...
  otu.table.polda <- t(otu.table.sim.filter)
  sam.ID <- paste0("ID", c(1:n.sam))
  colnames(otu.table.polda) <- sam.ID
  meta_data <- data.frame(sam.ID = sam.ID, Y = Y)
  res.polda <- polda(otu_table=otu.table.polda, metadata=meta_data, covar="Y", alpha=0.2)
  pval_df <- res.polda$P_Value
  pval_df$Taxon <- as.numeric(pval_df$Taxon)
  pval_df <- pval_df %>% arrange(Taxon)
  otu.polda <- summarize_otu_results(qvalue=pval_df$qval,
                                     causal.otus = causal.otus.filter, noncausal.otus = noncausal.otus.filter,
                                     fdr.target = 0.1)
...
p.global.all <- data.frame("LOCOM" = res.locom$p.global, "PERMANOVA-half" = res.adonis2["Y", 5])

if (scenario == "binary_Y_only") {
    otu.all <- rbind(unlist(otu.locom), unlist(otu.polda), unlist(otu.aldex2), unlist(otu.ancom), unlist(otu.ancombc), unlist(otu.dacomp), unlist(otu.wrench), unlist(otu.tss), unlist(otu.pseudo.half))
    rownames(otu.all) <- c("LOCOM", "POLDA", "ALDEx2", "ANCOM", "ANCOM-BC", "DACOMP", "WRENCH", "Wilcox-TSS", "Wilcox−alr−half")
} else if (scenario == "binary_Y_binary_C" | scenario == "binary_Y_continuous_C") {
    otu.all <- rbind(unlist(otu.locom), unlist(otu.aldex2), unlist(otu.ancom), unlist(otu.ancombc))
    rownames(otu.all) <- c("LOCOM", "ALDEx2", "ANCOM", "ANCOM-BC")
} else if (scenario == "continuous_Y_only") {
    otu.all <- rbind(unlist(otu.locom), unlist(otu.polda), unlist(otu.aldex2), unlist(otu.ancombc), unlist(otu.dacomp))
    rownames(otu.all) <- c("LOCOM", "POLDA", "ALDEx2", "ANCOM-BC", "DACOMP")
} else if (scenario == "continuous_Y_binary_C" | scenario == "continuous_Y_continuous_C"){
    otu.all <- rbind(unlist(otu.locom), unlist(otu.aldex2))
    rownames(otu.all) <- c("LOCOM", "ALDEx2")
}
otu.all <- as.data.frame(otu.all)
colnames(otu.all) <- c("n.otu", "sen", "sep", "fdr")



# Output
output_folder <- file.path('/scratch/ligen_root/ligen0/wangmk/LOCOM_simulation_test',
                           scenario, sprintf("%.1f", beta))

# p.global.summary <- Reduce("+", lapply(p.global.all, function(x) as.numeric(x < 0.05)))/length(p.global.all)
filename.p.global <- paste("S_", scenario, "_cau", causal.type,  "_n", n.sam, "_filter", filter.thresh, "_fdr", fdr.target, 
                           "_beta", beta, "_replicate", i.seed, "_global.txt", sep = "")
write.table(p.global.all, file = file.path(output_folder, filename.p.global))

# otu.summary <- Reduce("+", otu.all)/length(otu.all)
filename.otu <- paste("S_", scenario, "_cau", causal.type, "_n", n.sam, "_filter", filter.thresh, "_fdr", fdr.target, 
                      "_beta", beta, "_replicate", i.seed, "_otu.txt", sep = "")
write.table(otu.all , file = file.path(output_folder, filename.otu))
# 
